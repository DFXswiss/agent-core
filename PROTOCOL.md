...
Website-created person pings use synthetic origin `web:<login>`. Ack records a `web:<login>` event, does not transfer replica ownership of the ping row, and does not consume the sender device's `origin_seq`.
